...
**实施状态（2026-09-11，C5-9 第二批）**: 设置与治理面文案抽离已完成并通过本地门禁。本批覆盖 `features/cost/CostGovernancePage`、`features/quotas/{ProjectQuotasPage,QuotaMetricRow}`、`features/audit/{AuditLogsPage,AuditLogRow}`、`features/members/{MembersPage,InvitationAcceptPage}` 与 `features/shell/AnnouncementBanner`：全部标签、按钮、空状态、错误提示与 aria 文案改为词典键（zh/en 逐键镜像），页面标题不再是遗留英文品牌串（`AgentCanvas Cost Governance|Audit Log|Quotas` → 与导航同名），审计动作名改成 `audit.action.<id>` 的**类型化清单**（未知动作回落到原始 id 而非泄漏词典键），配额周期/成员加入时间/告警计数改按 locale 选 `zh-CN`/`en-US` 的数字与日期格式。代价与边界：`en.ts`/`zh.ts` 增量让初始 gzip 从 103.15 KiB 升到 108.37 KiB（限值 120 KiB，后续批次接近限值时应改按 feature 拆 chunk）；仍硬编码中文的约 80 个文件（canvas/workflows/knowledge/evaluations/apps/chat/models/admin/mcp 等）与后端英文 `detail` 直出中文 UI 的问题留待后续批次——仍沿用"改到哪个文件就整文件搬完，不留半翻译"的约定。受影响 e2e 断言（cost-governance/audit-log/project-quotas/platform-shell 的标题断言）同步改为新中文标题，并新增默认套件内的 `e2e/usage-export.spec.ts`。
