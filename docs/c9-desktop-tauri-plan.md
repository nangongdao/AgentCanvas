...
**C9-1 实施状态(2026-09-12)**:desktop profile、外壳骨架(supervisor 子 crate + Job Object 回收 + 单实例)、PyInstaller onedir(78.8 MiB,readyz 200 冒烟 2/2)、启动画面(readyz 分项流式渲染 + 失败面)、运输层(全流量直连,origin 注入)、安装包链路(NSIS + 体积门 + CI `desktop-build-windows`)、字体自托管(1.03 MiB 变量子集,零 CDN 依赖)均已落地并各自带门禁证据。**待 CI 安装包产出后执行**下方 14. 干净机验收清单,即可关闭 C9-1。

### 14. 干净机人工验收清单(C9-1 关闭条件,§8.3)

以下清单在 CI `desktop-build-windows` 产出安装包后,于一台**未装 Python/Node/Docker** 的干净 Windows 机(或干净用户账户)逐项人工执行:

1. **首装**:双击 NSIS 安装包(未签名 → SmartScreen 提示,记录处理步骤)→ 安装完成 → 双击图标 → splash 出现并逐项点亮 readyz 检查 → 主窗打开且概览页有数据;记录从双击到可交互的秒数(门:< 5s 可交互)。
2. **核心链路**:创建工作流 → 运行(mock provider)→ 流式输出可见 → 执行历史可回放;知识库上传 → 摄取 → 检索(SQL 向量后端)。
3. **进程回收**:关闭应用 → 任务管理器确认无 `agentcanvas-backend` 残留;再次启动正常。
4. **单实例**:再次双击图标 → 聚焦已有窗口,无第二实例。
5. **离线启动**:断网后启动应用 → 字体/界面正常(无 CDN 请求),无网络的功能可用。
6. **失败可见化**:临时将 `APP_DATA_DIR` 指向不可写路径启动 → splash 显示失败 + 可复制诊断 + 日志路径。
7. **升级路径**:旧数据目录(前一版本)→ 新版本启动 → 迁移自动执行 → 历史数据可见;失败场景见运行册 §9。
8. **卸载残留**:卸载后检查安装目录与 `%APPDATA%\AgentCanvas`(数据目录有意保留,记录卸载器行为与文档一致性)。
9. **窗口交互**:标题栏双击最大化、Snap Layouts、最小/最大尺寸约束、托盘行为(若该片已交付)。
10. **记录**:安装包体积(体积门输出)、首启秒数、遗留问题,回填本文档与 CHANGELOG。
...
**已决策(2026-09-12)**:桌面版**保留嵌入功能**,范围如下——
- 保留:应用详情页的 iframe/气泡**代码片段生成与复制**(纯前端能力,零后端依赖);
- 不保留:壳内 iframe 预览与 `app-embed.spec.ts` 的桌面通道执行(嵌入的目标环境是外部网站,壳内预览不代表真实投放面,且 Chromium 私网访问限制在本机 harness 中本就要求同回环源——桌面通道不为其维护特例);
- 依据:embed 的价值在「给外部网站一段可粘贴代码」,生成器在桌面形态下完整可用;壳内预览不是产品承诺面。
