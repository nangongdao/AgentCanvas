"""Tests for Azure OpenAI provider stream mappings."""
...
async def test_azure_openai_provider_streams_text_and_usage(monkeypatch) -> None:
    events = [
...
async def test_azure_openai_provider_sends_messages_and_tools(monkeypatch) -> None:
...
async def test_azure_openai_provider_surfaces_tool_calls(monkeypatch) -> None:
    events = [
        "data: " + _json.dumps({
            "choices": [{
                "delta": {
...
def test_azure_openai_provider_registered() -> None:
    from app.providers import PROVIDERS

    assert "azure_openai" in PROVIDERS


def test_azure_openai_provider_requires_base_url() -> None:
    with pytest.raises(ValueError, match="requires base_url"):
        AzureOpenAIProvider(model="gpt-4o", api_key="fake-key")


async def test_azure_openai_provider_custom_api_version(monkeypatch) -> None:
    captured: dict[str, Any] = {}
    ...
    _ = [c async for c in provider.stream_chat(
